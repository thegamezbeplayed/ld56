class Entity {
  ...
  public var angle(default,set): Int = 0;
  inline function get_angle() return angle;
  inline function set_angle(v:Int){angle=v==360?0:v; return angle;}
  /** Grid X coordinate **/
  public var cx = 0;
  /** Grid Y coordinate **/
  public var cy = 0;
  ...
  var _victims : FixedArray<Entity> = new FixedArray(20); // alloc cache
  /**
		Constructor
   **/
  public function new() {
  ...
  function getInRadius(e:Entity, rad:Float, ?inner=false){
    if( e!=this && Math.abs(cx-e.cx) <= 4 && Math.abs(cy-e.cy) <= 4 ) {
      var dist = Math.sqrt( (e.xx-xx)*(e.xx-xx) + (e.yy-yy)*(e.yy-yy) );
      /*if( ui.Console.ME.hasFlag(F_Debug) )
				trace('distance to ${e.name}: ${dist}');
				*/
      if( dist <= rad+e.radius ) return dist;
    }
    return -1;
  }
  /** Inflict damage **/
  ...
  public function grab(from:Null<Entity>){
    setAffectS(Absorb,2);
    from._victims.push(this);
  }
  ...
  public function bumpAwayFrom(e:Entity, velocity:Float) {
    ...
    var bdx = Math.cos(a)*velocity;
    var bdy = Math.sin(a)*velocity*0.66; // 2.5D z distortion
    vBump.addXY(bdx,bdy);
  }
  ...
  function useLdtkEntity(d:Entity_Mob) {
    setPosCase(d.cx, d.cy);
    xr = 0.5;
    yr = 1;
    var frict = d.f_mass/(d.f_size + d.f_mass);
    vBase.setFricts(frict,frict);

    mass = d.f_mass;
    wid = wid*d.f_size;
    hei = hei*d.f_size;
    sprScaleX = wid/Const.GRID;
    sprScaleY =  hei/Const.GRID;
  }
